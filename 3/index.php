 ...
 */
/*
$app->get('/csv/:ip', function($ip) {

$servername = "phpmyadmin.ctb9ppk1lc0s.eu-central-1.rds.amazonaws.com:3306";
$username = "phpMyAdmin";
$password = "phpMyAdmin";
$dbname = "ip2location";
...
$sql = "SELECT * FROM ip2location_db11 WHERE '$ip' BETWEEN ip_from AND ip_to";
...
for ( $i = 0; $i < $fields; $i++ )
{
    $header .= mysqli_fetch_field_direct( $result , $i ) . "\t";
}
echo "a";
/*
...
$bd_servidor = "";
if ($valor == 0) {
 $bd_servidor = "phpmyadmin.ctb9ppk1lc0s.eu-central-1.rds.amazonaws.com:3306";
}
else if ($valor == 1) {
 $bd_servidor = "phpmyadmin2.ctb9ppk1lc0s.eu-central-1.rds.amazonaws.com:3306";
} 
else {
 $bd_servidor =  "phpmyadmin-replica.ctb9ppk1lc0s.eu-central-1.rds.amazonaws.com:3306";
}
...
$conn = mysqli_connect(BD_SERVIDOR, BD_USUARIO, BD_PASSWORD, BD_NOMBRE);
...
$app->get('/json/:ip', function($ip) use($conn){
	...
	$sql = "SELECT country_code, country_name, region_name, city_name, zip_code, time_zone, latitude, longitude FROM ip2location_db11 WHERE " . $ip_sql . " BETWEEN ip_from and ip_to";
...
$app->get('/xml/:ip', function($ip) use($conn){
	...
	if (mysqli_num_rows($result) > 0) {
		...
		foreach ($datos as $clave => $valor){
			...
			echo $etiqueta;
			$xml = $xml . "<" . $etiqueta . ">" . $valor . "</" . $etiqueta . ">";
		}
		$xml = $xml . "</Response>";
		echo $xml;
	}
